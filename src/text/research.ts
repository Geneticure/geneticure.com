	...
	[
		...
		[
			[
				...
				`https://www.gavinpublishers.com/articles/Research-Article/Hypertension-An-Open-Access/Association-of-a-Multi-Gene-Panel-with-Blood-Pressure-Medication-Success-in-Patients-with-Hypertension-A-Pilot-Study`,
			],
			[
				`economics`,
			...
			[
				...
				`https://doi.org/10.1177/1179546819845883`,
			],
			[
				`algorithm`,
			...
			[
				...
				`https://doi.org/10.1016/j.yamp.2021.06.005`,
			],
			[
				`ace2`,
				...
				`https://doi.org/10.2217%2Fpgs-2020-0048`,
			],
			[
				`developing`,
				...
				`https://doi.org/10.2217/pgs-2019-0056`,
			],
		],
	],
	[
		...
		[
			...
			[
				...
				`https://faseb.onlinelibrary.wiley.com/doi/abs/10.1096/fasebj.2019.33.1_supplement.819.5`,
			],
			[
				`algorithm`,
				`Relationship between a weighted multi-gene algorithm and blood pressure control in hypertension.`,
				...
				`https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6463118/`,
			],
		],
	],
	[
		...
		[
			[
				...
				`Clinicaltrials.gov identifier: NCT02524873. Completed.`,
				`https://clinicaltrials.gov/ct2/show/NCT02524873`,
			],
			[
				`pharmaco`,
				`Investigating the Use of Genetics to Guide Pharmacologic Therapy for Hypertension.`,
				`Clinicaltrials.gov identifier: NCT02988245. Completed.`,
				`https://clinicaltrials.gov/ct2/show/NCT02988245`,
			],
			[
				`algorithm`,
				`Association Between a Genetic Algorithm to Predict Hypertension Therapy and Response to Treatment.`,
				`Clinicaltrials.gov identifier: NCT03292900. Completed.`,
				`https://clinicaltrials.gov/ct2/show/NCT03292900`,
			],
		],
	],
	[
		...
		[
			...
			[
				...
				`https://pubmed.ncbi.nlm.nih.gov/1969567`,
			],
		],
	],
];
...
let index = 0;
export const research = _research.map(([[categoryId, title], citations]) => ({
	citations: citations.map(([id, title, description, url])=> ({
		description,
		full: `<cite>` + (url ? `<a href="${url}" target="_blank">${title}</a>` : `${title}`) + `</cite>\n<span>${description}</span>`,
		href: `cite-${categoryId}-${id}`,
		id: `${categoryId}-${id}`,
		index: ++index,
		title,
		url,
...
export const researchByIndex = research.map((category) => category.citations).flat();

export const researchById = Object.fromEntries(
	researchByIndex.map((citation) => [
		citation.id,
		citation,
	]),
);
